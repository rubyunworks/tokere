...
#  def t.start(match) ; %r{ \[ (.*?) \] }mx ; end
#  def t.stop(match)  ; %r{ \[ [ ]* (#{resc(match[1])}) (.*?) \. \] }mx ; end
  def t.start(state) ; %r{ \[ (.*?) \] }mx ; end
  def t.stop(state)  ; %r{ \[ [ ]* (#{resc(state.last.match[1])}) (.*?) \. \] }mx ; end

  tokens << t

  t = TokenParser::UnitToken.new( :TWO )
#  def t.start(match) ; %r{ \& (.*?) \; }x ; end
  def t.start(state) ; %r{ \& (.*?) \; }x ; end
  tokens << t
